...
from game.boolean.signaltree import OutputReceiver, HiddenSignaler
...
class GlassTree(Sequential):
    ...
    def __init__(self, input_size: int = INPUT_SIZE) -> None:
        ...
        self.agent_b = OutputReceiver(
            input_size=self.input_size + 1
        )  # input list plus result of agent_a
    ...
    def forward(self, x: list[State]) -> State:
        ...
        if len(x) != self.input_size:
            raise Exception(
                f"length of input passed ({len(x)}) does not match network input size ({self.input_size})."
            )
